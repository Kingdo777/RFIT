...
{
    default_logger->debug("S - gettimeofday - {} {}", tvPtr, tzPtr);
...
{
    default_logger->debug("S - nanosleep - {} {}", reqPtr, remPtr);
...
WAVM_DEFINE_INTRINSIC_FUNCTION(wasi,
                               ...
                               I32 resNEvents)
{
    default_logger->debug("S - poll_oneoff - {} {} {} {}",
                                      subscriptionsPtr,
                                      eventsPtr,
                                      nSubs,
                                      resNEvents);
    ...
    for (int i = 0; i < nSubs; i++) {
        ...
        if (thisSub->type == __WASI_EVENTTYPE_CLOCK) {
            ...
            // Do the sleep
            timespec t{};
            RFIT_NS::utils::nanosToTimespec(timeoutNanos, &t);
                               ...
                               I32 resultPtr)
{
    ...
    uint64_t result = RFIT_NS::utils::timespecToNanos(&ts);
