...
namespace RFIT_NS::wasm {
    ...
    static void instantiateBaseModules() {
        ...
        RFIT_NS::utils::UniqueLock lock(baseModuleMx);
...
//            tearDown();
        ...
        if (other._isBound) {
            ...
            // clone memory
            compartment = Runtime::cloneCompartment(other.compartment, "", !restoreFromBaseSnapshot);
...
//        tearDown();
    ...
    void WAVMWasmModule::bindToFunction() {
        doBindToFunction(true);
    }

    void WAVMWasmModule::bindToFunctionNoZygote() {
        doBindToFunction(false);
    }

    void WAVMWasmModule::doBindToFunction(bool executeZygote) {
        /*
        ...
        moduleInstance =
                createModuleInstance(getFuncStr(), "");
    ...
    Runtime::Instance *WAVMWasmModule::createModuleInstance(
        ...
        IR::Module &irModule = getModule(sharedModulePath)->ir;
        ...
        if (isMainModule) {
            ...
            // dynamic module
            U64 nTableElems = getDynamicModuleTableSize(sharedModulePath);
            ...
            size_t dataSize = getDynamicModuleDataSize(sharedModulePath);
        ...
        Runtime::ModuleRef compiledModule = getModule(sharedModulePath);
 ...
 */
    bool WAVMWasmModule::execute(RFIT_NS::Message &msg) {
        const std::shared_ptr<spdlog::logger> &logger = RFIT_NS::utils::getLogger();
        if (!_isBound) {
            throw std::runtime_error(
        ...
        if (msg.isping()) {
            ...
            funcInstance = getFunction(moduleInstance, msg.funcptr(), false);
    ...
    U32 WAVMWasmModule::growMemory(U32 nBytes) {
        ...
        RFIT_NS::utils::WriteLock lock(moduleMemoryMutex);
...
//        IR::Module &module = moduleRegistry.getModule(boundUser, boundFunction, "");
    ...
    Runtime::Function *WAVMWasmModule::getFunctionFromPtr(int funcPtr) {
        ...
        if (funcObj == nullptr) {
            RFIT_NS::utils::getLogger()->error("Function pointer not found {}",
                                               funcPtr);
    ...
    WAVM::Runtime::ModuleRef WAVMWasmModule::getModule(const string &path) {
        if (path.empty()) {
            return mainModule;
        } else {
            if (dynamicPathModule.count(path) == 0) {
                WAVM::IR::Module moduleIR;
                // Feature flags
                moduleIR.featureSpec.simd = true;
                dynamicPathModule[path] = makeModule(utils::readFileToBytes(path));
                auto &module = dynamicPathModule[path]->ir;
                ...
                this->originalTableSizes[path] =
                        module.tables.imports[0].type.size.min;

                module.tables.imports[0].type.size.min =
                        (WAVM::U64) mainModule->ir.tables.defs[0].type.size.min;
                module.tables.imports[0].type.size.max =
                        (WAVM::U64) mainModule->ir.tables.defs[0].type.size.max;
            }
            return dynamicPathModule[path];
        }
    }

    size_t WAVMWasmModule::getDynamicModuleDataSize(const string &path) {
        assert(!path.empty());
        WAVM::IR::Module &irModule = getModule(path)->ir;
        size_t dataSize = 0;
        for (const auto &ds : irModule.dataSegments) {
            dataSize += ds.data->size();
        }
        return dataSize;
    }

    WAVM::U64 WAVMWasmModule::getDynamicModuleTableSize(const string &path) {
        assert(originalTableSizes.count(path) != 0);
        return originalTableSizes[path];
    }
}
